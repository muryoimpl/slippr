...
class ProgressBar extends React.Component {
  ...
  registerIpcEvents() {
    ...
    ipcRenderer.on('start-timer-in-page', (_event, json) => {
      ...
      store.dispatch(progressBarActions.setElapsedIntervalId(id));
    });
  ...
  render() {
    const {
      progress, elapsedSeconds, totalSeconds, showIcons,
    } = this.props;

    return (
      <div className="p-proggress-area">
        { totalSeconds !== 0 && showIcons
          && (<span className="p-progress-icon-area" style={{ width: `${progress}%` }}><img alt="koko" src="assets/images/emoji/koko.png" className="p-progress-icon" /></span>)
        }
        { totalSeconds !== 0 && showIcons
          && <span className="p-progress-icon-area" style={{ width: `${calcTimeProgress(elapsedSeconds, totalSeconds)}%` }}><img alt="hourglass" src="assets/images/emoji/hourglass_flowing_sand.png" className="p-progress-icon" /></span>
        }
        <progress className="p-progress-bar" max={Settings.MAXIMUM_PROGRESS} value={progress} />
      </div>
    );
  }
}
