...
class Timer extends React.Component {
  ...
  tick() {
    const { store } = this.context;
    const {
      hours, minutes, seconds, intervalId, limit,
    } = this.props;
  ...
  render() {
    const {
      limit, hours, minutes, seconds, started, blink,
    } = this.props;
        ...
        <form className="p-timer__time mgl">
          <div className="form-group">
            ...
            <input
              ...
              step="1"
            />
            <button type="button" className="btn btn-default mgl" onClick={e => this.handleTimerReset(e)} disabled={started}>RESET</button>
            <button type="button" className="btn btn-default mgl" onClick={e => this.handleClearTimer(e)} disabled={started}>CLEAR</button>
            { !started
              && <button type="button" className="btn btn-primary mgl p-timer__start" onClick={e => this.handleTimerStart(e)} disabled={isLimitInvalid}>START</button>
            }
            { started
              && <button type="button" className="btn btn-negative mgl" onClick={e => this.handleTimerStop(e)}>STOP</button>
            }
          </div>
        </form>
      </div>
